...
function renderMgFR(){const p=$('#panelFr'),sites=D.frequentSites||[];if(!sites.length){p.innerHTML='<div class="mg-empty">暂无高频使用记录</div>';return}const fl=curFreqLabel||'次/周';p.innerHTML=sites.map((s,i)=>`<div class="mg-item" data-i="${i}"><div class="mg-icon" style="background:${s.color}">${esc(s.letter)}</div><div class="mg-info"><div class="mg-name">${esc(s.title)}</div><div class="mg-url">${esc(s.url)} · ${s.freq} ${fl}</div></div><button class="mg-add-sc" data-title="${esc(s.title)}" data-url="${esc(s.url)}" data-color="${s.color}" data-letter="${esc(s.letter)}" title="添加到快捷导航">+</button><button class="mg-del" data-act="df">✕</button></div>`).join('');p.querySelectorAll('[data-act="df"]').forEach(b=>b.addEventListener('click',async()=>{const item=b.closest('.mg-item');item.classList.add('removing');await new Promise(r=>setTimeout(r,400));await msg('removeFrequentSite',{index:+item.dataset.i});showToast('已删除');await refreshUI();renderMgFR();renderRecent()}));p.querySelectorAll('.mg-add-sc').forEach(b=>b.addEventListener('click',async()=>{const title=b.dataset.title,url='https://'+b.dataset.url,color=b.dataset.color,letter=b.dataset.letter;const r=await msg('addShortcut',{shortcut:{name:title,url,color,icon:letter,imgUrl:''}});if(!r.success){showToast(r.message||'添加失败');return}showToast(`已添加 "${title}" 到快捷导航`);await refreshUI();renderMgFR();renderRecent();renderMgSC();renderShortcuts()}))}
...
g.querySelectorAll('.ra').forEach(b=>b.addEventListener('click',async e=>{e.preventDefault();e.stopPropagation();const title=b.dataset.title,url='https://'+b.dataset.url,color=b.dataset.color,letter=b.dataset.letter;const r=await msg('addShortcut',{shortcut:{name:title,url,color,icon:letter,imgUrl:''}});if(!r.success){showToast(r.message||'添加失败');return}showToast(`已添加 "${title}" 到快捷导航`);await refreshUI();renderRecent($('#frSearchInput')?.value.trim()||'')}))}
